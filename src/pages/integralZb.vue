...
<template>
  <div>
    ...
    <div class="main">
      <!-- 导航栏 -->
      <head-menu :index="'/integralZb'"></head-menu>
      <!-- 标题栏 -->
      <div class="title_bar">
        <el-row>
          ...
          <el-col :span="6" class="title_bar_font">
            <el-radio-group v-model="radio1" @change="changeListType" style="float:right">
              ...
              <el-radio-button label="人员列表"></el-radio-button>
            </el-radio-group>
            <!-- <el-button type="primary" plain @click="changeListType" v-if="listType == 1">人员列表</el-button>
            <el-button type="primary" plain @click="changeListType" v-if="listType == 2">项目列表</el-button>-->
          </el-col>
        </el-row>
      </div>
      <!-- 主内容 -->
      <div class="table_main">
        <div v-if="listType == 1">
          <el-row class="table_title">
            <el-col :span="20" style="height: 45px;">
            ...
            <el-col :span="4">
              <el-radio-group v-model="radioXm" @change="searchQq">
                <el-radio-button label="有效项目"></el-radio-button>
                <el-radio-button label="过期项目"></el-radio-button>
              </el-radio-group>
          ...
          <el-table :data="project.list" border style="width: 100%" key="projectList" v-loading="project.loading">
            ...
            <el-table-column label="操作" width="150" align="center">
              <template slot-scope="scope">
                ...
                <!-- <el-button type="danger" @click="delXmRyList(scope.row)" size="mini">删除</el-button> -->
              </template>
            </el-table-column>
          </el-table>
        ...
        <div v-if="listType == 2">
          ...
          <el-table :data="ry.list" border style="width: 100%;margin-top:10px;" key="ryList" v-loading="ry.loading">
            ...
            <el-table-column prop="total_integral" label="总积分" align="center">
              <template slot-scope="scope">
            ...
            <el-table-column prop="spend_integral" label="花费积分" align="center">
              <template slot-scope="scope">
            ...
            <el-table-column prop="total_integral" label="剩余积分" align="center">
              <template slot-scope="scope">
    ...
    <!-- 添加项目弹窗 -->
    <el-dialog title="添加项目" :visible.sync="GsListVisible" top="30px" width="70%" :close-on-click-modal="close_on_click_modal">
      ...
      <div class="title_bottom">
        <el-button type="" @click="GsListVisible = false">关闭</el-button>
      </div>
    </el-dialog>
    <!-- 添加人员弹窗 -->
    <el-dialog title="添加人员" :visible.sync="RyListVisible" top="50px" width="70%" :close-on-click-modal="close_on_click_modal">
      <el-tabs v-model="activeName" @tab-click="handleClick">
        <el-tab-pane label="内部人员" name="内部人员"></el-tab-pane>
        <el-tab-pane label="外部人员" name="外部人员"></el-tab-pane>
      </el-tabs>
      <el-form :inline="true" class="demo-form-inline" style="float:right;" @submit.native.prevent>
      ...
      <el-table :data="add.RyList" border style="width: 100%;margin-top:10px;" v-loading="add.ryloading">
        <el-table-column prop="id" label="序号" width="60" align="center">
      ...
      <div class="title_bottom">
        <el-button type="" @click="RyListVisible = false">关闭</el-button>
      </div>
    </el-dialog>
    <!-- 项目查看窗口 -->
    <el-dialog :title="project.title" :visible.sync="project.DetVisible" width="70%" top="30px" :close-on-click-modal="close_on_click_modal">
      ...
      <div class="title_bottom">
        <el-button type="" @click="project.DetVisible = false">关闭</el-button>
      </div>
    </el-dialog>
    <!-- 人员查看窗口 -->
    <el-dialog title="查看人员" :visible.sync="ry.DetVisible" width="70%" :close-on-click-modal="close_on_click_modal">
      <el-table key="aaa" :span-method="arraySpanMethodRy" border style="margin-top:10px" :data="ry.ck.list">
        <el-table-column width="60" type="index" label="序号" align="center"></el-table-column>
        <el-table-column align="center" prop="xmmc" label="项目名称" width="500px"></el-table-column>
        <el-table-column align="center" prop="gsmc" label="公司名称" width="300px"></el-table-column>
        ...
        <!-- <el-table-column label="操作" align="center">
          <template slot-scope="scope">
            <el-button type="primary" @click="handGsRy(scope.row)" size="mini">{{scope.row.allsure}}</el-button>
          </template>
        </el-table-column> -->
        <el-table-column label="操作" align="center">
          <template slot-scope="scope">
      ...
      <div class="title_bottom">
        <el-button type="" @click="ry.DetVisible = false">关闭</el-button>
      </div>
    </el-dialog>
    <!-- 业绩增加积分窗口 -->
    <el-dialog :visible.sync="project.zj.visible" :title="project.zj.xmmc" width="50%" :close-on-click-modal="close_on_click_modal">
      <el-form ref="form" :model="project" label-width="80px" @submit.native.prevent>
    ...
    <!-- 人员增加积分窗口 -->
    <el-dialog :visible.sync="ry.zj.visible" :title="ry.zj.title" width="50%" :close-on-click-modal="close_on_click_modal">
      <el-form ref="form" :model="ry" label-width="80px" @submit.native.prevent>
        <el-form-item label="积分(万)">
    ...
    <el-dialog :visible.sync="ry.mx.visible" :close-on-click-modal="close_on_click_modal">
      ...
      <div style="text-align:center">
        ...
        <el-pagination background layout="prev, pager, next" :total="ry.mx.total" :current-page="ry.mx.page" @current-change="ryMxChangePage"></el-pagination>
      </div>
      <div class="title_bottom">
    ...
    <!-- 查询人员对公司积分的操作 -->
    <el-dialog :visible.sync="ry.gsry.visible" title="确认积分" top='10px' :close-on-click-modal="close_on_click_modal" width="60%">
      <div style="text-align:center">
        <el-form :inline="true" :model="addIntegralForm" class="demo-form-inline" style="text-align:right;">
          <el-form-item label="积分">
            <el-input v-model="addIntegralForm.integral" placeholder="积分" type="number"></el-input>
          </el-form-item>
          <el-form-item label="备注">
            <el-input v-model="addIntegralForm.mark" placeholder="备注" style="width:500px;"></el-input>
          </el-form-item>
          <el-form-item>
      ...
      <el-dialog width="50%" title="编辑积分" :visible.sync="ry.gsry.innerVisible" append-to-body :close-on-click-modal="close_on_click_modal">
        <el-form ref="form" :model="ry.gsry.form" label-width="80px" @submit.native.prevent>
          <el-form-item label="积分">
...
export default {
  ...
  methods: {
    addIntegral() {
      console.log('new_user_id:' + this.new_user_id)
      let params = {
        integral: this.addIntegralForm.integral,
        txt: this.addIntegralForm.mark,
        user_id: this.new_user_id,
        order_id: this.$store.state.userInfo.user_id,
        gs_id: this.ry.gsry.gs_id,
        jbxx_id: this.project.ck.jbxx_id
      }
      if (!params.integral) {
        this.$message.error('你还没填积分')
        return
      }
      this.axios.post('/Index/integral/addCompanyIntegral', params, data => {
        if (data.status == 'success') {
          this.addIntegralForm.integral = ''
          this.addIntegralForm.mark = ''
    ...
    arraySpanMethodRy({ row, column, rowIndex, columnIndex }) {
      let prop = column.property
      if (prop && prop != 'company_integral' && prop != 'gsmc') {
        //如果存在prop
        const _row = this.merge_array[rowIndex][prop]
        const _col = _row > 0 ? 1 : 0
        return {
    ...
    getRyList() {
      var data = {
        ...
        service_cate: this.activeName,
        order: 'asc'
      }
      this.add.ryloading = true
      this.axios.post('/index/integral/peopleLists', data, data => {
        this.add.ryloading = false
        this.add.RyList = data.rows || []
        this.add.RyListTotal = data.total || 0
      })
    },
    //查询添加列表
    addSearch(type) {
    ...
    handGsRy(row) {
      ...
      this.axios.post('/Index/integral/companyDetails', ajaxdata, data => {
        ...
        this.project.ck.jbxx_id = ajaxdata.jbxx_id
        this.peopleCheckList()
        console.log('jbxx_id：' + this.project.ck.jbxx_id)
      })
    }
  }
}
