  ...
  <div class="ItemInfo_content">
    <div v-if="allVisible">
      ...
      <div class="content_header">
        <div class="tittle">{{itemInfo.gsmc}}
          <div style="font-size:35px;position:absolute;right:67px;color:#409eff;top:19px;cursor:pointer" @click="sharePage()">
            ...
            <div class="table_footer">
              <el-row :gutter="20">
                ...
                <div :style="tableStyleL" @click="checkTable(1)">
                  <p :style="{marginBottom:'20px',width:'100%',textAlign:tableIndex==1?'center':'left',fontSize:'20px',color:'#67C23a'}">项目管理机构组织</p>
                  <el-button type="primary" @click="openAddRy" style="float:right;margin-left:20px;" v-if="unlock">添加</el-button>
                  <!--左边的表-->
                  <ry-list :needData="needData" :unLock="unlock" :isShare="isShare"></ry-list>
                </div>
                <div :style="tableStyleR" @click="checkTable(2)">
                  <p :style="{marginBottom:'20px',width:'100%',textAlign:tableIndex==2?'center':'right',fontSize:'20px',color:'#67C23a'}">类似项目业绩</p>
                  <!--右边的表-->
                  <el-table border :data="gsDataR.gslist" v-loading="gsDataR.loading">
                    ...
                    <el-table-column label="审核" prop="sh" align="center" width="100" v-if="unlock">
                      <template slot-scope="scope">
      ...
      <div style="background-color: #f5f7fa">
        <el-dialog :title="liuyan.title" :visible.sync="liuyan.visible" width="35%" :close-on-click-modal="false">
          <leave-word :needData="leaveNeedData"></leave-word>
        </el-dialog>
      </div>
      <!-- 企业不通过原因 -->
      <el-dialog title="业绩驳回" :visible.sync="qynoPassDialogVisible" width="35%" :close-on-click-modal="false">
        ...
        <div>
          <el-input type="textarea" @keyup.ctrl.enter.native="noPassOk()" :rows="4" placeholder="其他驳回原因(例：该业绩无中标公示，无竣工验收)" v-model="noPass.other"></el-input>
        </div>
      ...
      <el-dialog title="业绩驳回记录" :visible.sync="noPass.Visible" width="60%">
        ...
        <el-pagination style="margin:20px 0" ref="fenye" @current-change="pageSizeChangeNoPass" align="right" background layout="total,prev, pager, next" :total="gsDataNoPass.total"></el-pagination>
      </el-dialog>
      <!--点击业绩名称弹窗-->
      <el-dialog title="详细信息" :visible.sync="xq.Visible" width="90%">
        ...
        <div>
          <el-table border :data="xq.list1">
            ...
            <el-table-column prop="xmmc" label="项目名称" align="center">
              <template slot-scope="scope">
                <a :href="scope.row.link" target="_blank">{{scope.row.xmmc}}</a>
              </template>
            </el-table-column>
    ...
    <el-dialog title="请输入密码" :visible.sync="pasVisible" width="500px">
      <el-input v-model="passInput" placeholder="请输入密码" @keyup.enter.native="sharePasOk()"></el-input>
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="sharePasOk()">确 定</el-button>
        <el-button @click="pasVisible = false">取 消</el-button>
      </span>
    </el-dialog>
      ...
      <div>
        <el-table border :data="addTableList" v-loading="addloading">
          ...
          <el-table-column label="姓名" prop="xm" align="center">
            <template slot-scope="scope">
              <a style="color: rgb(58, 142, 230)" v-if="scope.row.http" :href="scope.row.http" target="_blank">{{scope.row.xm}}</a>
              <span style="color: #606266" v-else :href="scope.row.http">{{scope.row.xm}}</span>
            </template>
          </el-table-column>
    ...
    <el-dialog title="分享" :visible.sync="shareVisible" width="500px">
      ...
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="copyPagePas()">复制到粘贴板</el-button>
...
export default {
    ...
    methods: {
        ...
        sharePasOk() {
            this.axios.post(
                "/index/Human_Review/checkProject",
                {
                    jbxx_id: this.$route.query.jbxx_id,
