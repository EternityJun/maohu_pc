  ...
  <div>
    ...
    <div class="plan-top">
      ...
      <div style="display:inline-block;margin:0 20px">
        ...
        <el-select @change="onChangePLan" size="medium" v-model="nowPlanData.selectVal">
          <el-option v-for="item in planList2" :key="item.id" :label="item.node_name" :value="item.status_id"></el-option>
        </el-select>
      </div>
      <!-- <span>已选方案:</span>
      <label>{{nowPlanData.node_name}}</label> -->
    </div>
          ...
          <div>
            ...
            <div style="flex:1">
              <el-table height="100%" border :data="yesGsData.tableList" v-loading="yesGsData.loading" @sort-change="yesTableSort" :row-style="tableRowClassName" @select-all="onSelect_yesGsRows" @select="onSelect_yesGsRows">
                ...
                <el-table-column prop="zbxxids" width="110px" label="中标次数" align="center" sortable="custom">
                  <template slot-scope="scope">
                    <el-button @click="open_detail(scope.row,'zbxxids',1)" type="text">{{scope.row.zbxxids}}</el-button>
                  </template>
                </el-table-column>
                <el-table-column prop="zbids" width="110px" label="投标次数" align="center" sortable="custom">
                  <template slot-scope="scope">
            ...
            <el-pagination style="margin-top:20px" @current-change="yesGsDataChangePage" @size-change="yesGsDataChangeSize" align="right" background layout="sizes,total,prev, pager, next" :total="yesGsData.total"></el-pagination>
          </div>
          <div>
            ...
            <el-pagination style="margin-top:20px" @current-change="noGsDataChangePage" @size-change="noGsDataChangeSize" align="right" background layout="sizes,total,prev, pager, next" :total="noGsData.total"></el-pagination>
          </div>
        </div>
      </carousel-item>
      <carousel-item>
        <div class="diplay-flex-column">
          ...
          <el-pagination style="margin-top:20px" @current-change="allGsDataChangePage" @size-change="allGsDataChangeSize" align="right" background layout="sizes,total,prev, pager, next" :total="allGsData.total"></el-pagination>
        </div>
      </carousel-item>
    </carousel-card>
    <div style="margin:50px 0">
      <carousel-card height="650px">
        <carousel-item>
          <div class="diplay-flex-column">
            <p class="table-title" style="color:#2f80d1">人工审核通过公司集合</p>
            <el-form inline size="medium" style="font-size:0" @submit.native.prevent>
              <el-form-item style="width:180px">
            ...
            <el-pagination style="margin-top:20px" @current-change="yesGetedGsChangePage" @size-change="yesGetedGsChangeSize" align="right" background layout="sizes,total,prev, pager, next" :total="yesGetedGsData.total"></el-pagination>
          </div>
        </carousel-item>
        <carousel-item>
          <div class="diplay-flex-column">
        ...
        <carousel-item>
          <div class="diplay-flex-column">
            ...
            <el-pagination style="margin-top:20px" @current-change="noGetedGsChangePage" @size-change="noGetedGsChangeSize" align="right" background layout="sizes,total,prev, pager, next" :total="noGetedGsData.total"></el-pagination>
          </div>
        </carousel-item>
      </carousel-card>
    </div>
...
export default {
    ...
    data() {
        return {
            ...
            last_treeId: this.$route.query.thread_id,
    ...
    methods: {
        ...
        open_detail(row, sortField, type) {
            ...
            this.zb_tb_count.data = {
                thread_id: this.nowPlanData.selectVal,
        ...
        ajaxXmInfo() {
            this.axios.post(
                ...
                data => {
                    ...
                    document.title = "审核-" + data.res.xmmc;
                }
            );
        },
        //认识的人计算
                    ...
                    if (data.status == "failed") {
                        ...
                        if (cate == 1) {
                            ...
                            if (filterObj.length > 0) {
                                this.nowPlanData.node_name =
                                    filterObj[0].node_name;
                                this.nowPlanData.selectVal =
                                    filterObj[0].status_id;
                            } else {
                                this.nowPlanData.node_name =
                                    data.rows[0].node_name;
                                this.nowPlanData.selectVal =
                                    data.rows[0].status_id;
                            }
                            this.ajaxYesGsList();
                            this.ajaxNoGsList();
                            this.getNode();
        ...
        onChangePLan() {
            ...
            this.noGsData.total = 1;
            this.noGsData.total = 0;
            let filterObj = this.planList2.filter(
                v => v.status_id == this.nowPlanData.selectVal
            );
            if (filterObj.length > 0) {
            ...
            this.ajaxNoGsList();
            this.getNode();
            this.axios.post("/index/Human_Allot/newPlan", {
                human_id: this.id,
                thread_id: this.nowPlanData.selectVal
            });
        },
        ...
        onDistribute(dataObj, ajaxObj) {
            ...
            if (this[dataObj].formData.ryVal) {
                userObj = this.allUsers[this.user_cate].filter(
