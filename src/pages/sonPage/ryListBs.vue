  ...
  <div class="ry_list">
    <el-table :data="tableLists" v-loading="loading">
      ...
      <el-table-column label="执业或职业资格证明" align="center">
        ...
        <el-table-column align="center" prop="zczsh" label="证书号" class-name="table_cell_padding0">
          <template slot-scope="scope">
            <div class="table_col_child">{{scope.row.zczsh}}</div>
            <div class="table_col_child" v-for="one in scope.row.other_arr" :key="one.id" :style="{color:one.expire-0?'':'#f56c6c'}">{{one.zczsh}}</div>
          </template>
        </el-table-column>
    ...
    <div style="text-align:center;margin-top: -50px;" v-if="page >= total/10">
      <el-button type="text" style="font-size:45px;color:rgb(103, 194, 58)" @click="openAdd" class="el-icon-circle-plus"></el-button>
    </div>
    <el-dialog title="人员列表" append-to-body :visible.sync="ryListVisible" width="80%">
      <iteminfo-peole :gsId="gs_id" :isShare="isShare" :btnType="btnType" :types="childrenType" :nowRow="childrenNowRow" :ry_id="ry_id" @onSelectNewuser="onSelectNewuser"></iteminfo-peole>
    </el-dialog>
    <!-- 查看业绩列表 -->
    <el-dialog title="业绩列表" :visible.sync="checkyjtk" width="80%">
...
export default {
  ...
  methods: {
    ...
    getRyList() {
      this.loading = true
      this.axios.post(
        '/index/Laipi_Ry_Yj/companyPersonList', //项目机构管理
        {
          gs_id: this.gs_id,
          page: this.page,
          row: 10,
      ...
      if (types == 'checkYj') {
        ...
        this.$confirm('此操作将删除该数据, 是否继续?', '提示', {
          ...
          .then(() => {
            ...
            this.axios.post('/index/Laipi_Ry_Yj/companyDelPerson', obj, data => {
              if (data.status == 'success') {
                let obj = row
                obj.cate = 3
