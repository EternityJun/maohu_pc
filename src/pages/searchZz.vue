<template>
  <div style="color:#333333">
    <div class="tittle">企业信息</div>
    <div class="content_div">
      <div class="top_tj">
        <span>企业属地</span>
        <el-select style="margin-right:6px;width:180px;" size="small" filterable v-model="selectValAll" placeholder="请选择" @change="onChange_area()">
          <el-option v-for="item in all_areaArray" :key="item.full_prov" :label="item.full_prov" :value="item.full_prov"></el-option>
        </el-select>
        <span style="margin-left:50px;">资质条件</span>
        <el-input style="width:234px" size="medium" readonly placeholder="请选择资质条件" @focus="onOpentk"></el-input>
      </div>
      <div class="search_result" v-if="zz_total_array.length != 0">
        <div class="left">
          <div class="text" @click="onClickAll">
            为你找到全部相关结果
            <span class="color_num">{{all_total}}</span>
            条
          </div>
          <div class="every_tj" v-for="item in zz_total_array" :key="item.name">
            <div class="every_name">{{item.name}}</div>
            <div class="every_th">
              <div>等级名称</div>
              <div>等级</div>
              <div>{{checked_area.prov}}</div>
              <div>{{checked_area.r_prov}}</div>
            </div>
            <div class="every_tr" v-for="item2 in item.value" :key="item2.name">
              <div class="tr_name">{{item2.name}}</div>
              <div>
                <div class="every_td" v-for="item3 in item2.value" :key="item3.dict">
                  <div>{{item3.level}}</div>
                  <div :data-dd="selected_item.dict+selected_item.dq +':::'+ item3.dict+checked_area.prov" :class="['color_num',selected_item.dict+selected_item.dq == item3.dict+checked_area.prov ? 'selected_num':'']" @click="onSelectItem(item3,checked_area.prov,item.name+' / '+ item2.name +' / '+ item3.level)">{{item3.prov}}</div>
                  <div :class="['color_num',selected_item.dict+selected_item.dq == item3.dict+checked_area.r_prov ? 'selected_num':'']" @click="onSelectItem(item3,checked_area.r_prov,item.name+' / '+ item2.name +' / '+ item3.level)">{{item3.r_prov}}</div>
                </div>
              </div>
            </div>
          </div>
