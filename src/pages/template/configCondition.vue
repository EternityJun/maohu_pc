        ...
        <el-col :span="12">
          ...
          <div class="tiaojian-content">
            ...
            <el-form :inline="true" :model="form.lsyj.ajax_data" :rules="rules" @submit.native.prevent>
              ...
              <div :style="{display:this.showAroundTime ?'inline-block':'none',verticalAlign:'bottom'}">
                <el-form-item>
                  ...
                  <el-date-picker value-format="yyyy-MM-dd" v-model="form.lsyj.ajax_data.date.maxTime" type="date" placeholder="选择日期" :picker-options="pickerOptions1"></el-date-picker>
                </el-form-item>
              </div>
            </el-form>
            <el-form inline style="padding-left:26px" :model="form.lsyj.ajax_data" :rules="rules" @submit.native.prevent>
              <el-form-item>
                <!-- 开工 -->
                <el-select v-model="value_kgwg" multiple placeholder="请选择" style="width:200px">
                  <el-option v-for="item in kgwg_select" :key="item.value" :label="item.label" :value="item.value">
                  </el-option>
                </el-select>
                <!-- <el-popover placement="bottom" trigger="click">
                                    <el-tree ref="treekgwg" @check="onSelect_kgwg" :default-checked-keys="form.lsyj.ajax_data.kgwg ? form.lsyj.ajax_data.kgwg.split(',') : []" :data="kgwg_select" show-checkbox node-key="value" :check-on-click-node="true"></el-tree>
            ...
            <el-form>
              <el-form-item label="关键词" label-width="100px">
                <el-input v-model.trim="form.lsyj.yjInputKey" style="width:250px" @keyup.enter.native="onConfirmKey" clearable placeholder="请输入关键字"></el-input>&nbsp;
                <label class="text_ipt">例：道、路、桥(回车键确定添加)</label>
                <div>
                  <el-tag v-for="(tag,index) in form.lsyj.ajax_data.YjTags" :key="tag.name" closable @close="onDelete_yjTag(index)">{{tag.name}}</el-tag>
                </div>
              </el-form-item>
              ...
              <el-form-item label="业绩来源" label-width="100px">
              ...
              <div style="display:flex;align-items:center" v-for="(xmjlForm,index) in form.otherry.selectedAll.zjgcs" :key="'a'+index">
                <el-form :inline="true" @submit.native.prevent>
                  <el-form-item label="造价工程师" class="margin_b10" label-width="110px">
                    <el-select v-model="xmjlForm.zczy" clearable placeholder="请选择" multiple>
            ...
            <div style="display:flex;align-items:center" v-for="(xmjlForm,index) in form.otherry.selectedAll.zcaqgcs" :key="index">
              <el-form :inline="true" @submit.native.prevent>
                <el-form-item label="注册安全工程师" class="margin_b10">
                  <el-select v-model="xmjlForm.zczy" collapse-tags placeholder="请选择" multiple @change="changeAll">
                ...
                <el-form-item label="证书号" class="margin_b10">
                  <el-select v-model="xmjlForm.zsh" clearable placeholder="请选择">
...
export default {
  ...
  watch: {
    parent: function() {
      ...
      this.node_id = this.parent_data.node_id
  ...
  created() {
    ...
    // this.isUpdate()
    //获取资质类型
    this.axios.post('/index/Aptitude/getDictionary', { types: 'zz' }, data => {
      this.options_zz = data.rows
    })
    this.axios.post('/index/Dictionaries/get_dict', { dict_id: 9 }, data => {
  ...
  methods: {
    ...
    onMinus_otherRow(index, typeTxt) {
      this.form.otherry.selectedAll[typeTxt] = this.form.otherry.selectedAll[
        typeTxt
      ].filter((v, i) => {
        if (i != index) {
          return v
    ...
    onMinus_row(index, typeTxt) {
      this.form[typeTxt].selectedAll = this.form[typeTxt].selectedAll.filter(
        (v, i) => {
          if (i != index) {
            return v
          }
    ...
    onSetyear() {
      let now = new Date()
      this.form.lsyj.ajax_data.year = Math.round(this.form.lsyj.ajax_data.year)
      this.form.lsyj.default_date =
        now.getFullYear() - parseInt(this.form.lsyj.ajax_data.year) + '-01'
    },
    // 删除对应的业绩标签
    ...
    onConfirmKey() {
      if (this.form.lsyj.yjInputKey) {
        ...
        var filterArray = this.form.lsyj.ajax_data.YjTags.filter((v, i) => {
    ...
    // 选择业绩来源
    onCheckOneChange(v) {
      this.form.lsyj.checkAll = v.length == this.form.lsyj.yjlyOptions.length
    ...
    saveYj(differBtn) {
      ...
      if (this.form.lsyj.lsyjyes) {
        let lsyj_data = {
          condition: {
            keys: {
              contain: this.form.lsyj.ajax_data.YjTags
                ? this.form.lsyj.ajax_data.YjTags.map(v => v.name)
                : ''
            },
            xmlx: {
          ...
          around: {
            minTime: this.form.lsyj.ajax_data.date.minTime || '',
            maxTime: this.form.lsyj.ajax_data.date.maxTime || '',
            kg: this.value_kgwg.indexOf('开工') != -1 ? true : false,
            wg: this.value_kgwg.indexOf('完工') != -1 ? true : false,
            number: this.form.lsyj.ajax_data.number || 0
          },
          jbxx_id: this.jbxx_id,
          thread_id: this.thread_id,
        ...
        lsyj_data.display.kgwg = self.value_kgwg.join(',')
    ...
    saveZj(differBtn) {
      ...
      if (this.form.otherry.otherryyes) {
        console.log(this.form.otherry.otherryyes)
      ...
      if (this.form.zz.zzyes) {
        ...
        this.axios.post(
          '/index/Aptitude/saveZz',
          {
            ...
            display: {
              orand: this.form.zz.ajax_data.orand,
              selected_zzContentL: this.form.zz.display.selected_zzContentL,
              selected_zzContentR: this.form.zz.display.selected_zzContentR
            },
            jbxx_id: this.jbxx_id,
            thread_id: this.thread_id
          },
    ...
    onConfirmrun(differBtn) {
      ...
      if (this.ajax_number == ajaxNum) {
        ...
        this.axios.post(
          ...
          { zzid: this.zz_id, node_id: this.node_id },
          data => {
            this.axios.post(
              '/index/Search_yj/searchYj',
              { yj_id: this.yj_id, node_id: this.node_id },
              data => {
