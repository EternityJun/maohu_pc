        ...
        <div class="evryone">
          ...
          <div class="second">
            <el-button
              size="small"
              v-if="user_btn.btn1"
              :disabled="user_btn.disable1"
              type="primary"
              plain
              @click="ConfigurationLook"
            >
              配置查看
            ...
            <el-button
              ...
              plain
              type="primary"
              :disabled="user_btn.disable2"
              @click="config_wj = true"
            >配置文件</el-button>
          </div>
        </div>
      </div>
            ...
            <div class="myrow">
              ...
              <div class="mycol_1">
                <el-button
                  size="mini"
                  type="primary"
                  v-if="!projectPerformanceHeads.zbxx_id"
                  :disabled="projectPerformanceHeads.status == '预定'"
                  @click="onClickPassFirst"
                >选择文件</el-button>
                <a
                  v-if="projectPerformanceHeads.zbxx_id"
                  style="color: #409EFF;"
                  :href="'http://www.520coc.cn/CompanyRounds/getMassage?zbgsid=&type=0&zbwj=undefined&zbqd=undefined&zbxxid='+projectPerformanceHeads.zbxx_id"
                  target="_blank"
                >点击查看</a>
              </div>
            </div>
          ...
          <div class="table_footer">
            <el-row :gutter="20">
                ...
                <el-table :data="tableDataFirst.rows" v-loading="loading1" border>
                  ...
                  <el-table-column prop="reserve" label="预订" align="center" min-width="12%">
                    <template slot-scope="scope">
                      <el-tooltip
                        class="item"
                        :disabled="!scope.row.reserve_txt"
                        effect="dark"
                        :content="scope.row.reserve_txt"
                        placement="top"
                      >
                        <el-button
                          type="text"
                          style="color:#606266"
                          @click="onShowTk('reserve_txt',scope.row)"
                        >
                          <i
                            :class="scope.row.reserve=='已预订'?'el-icon-circle-check':'el-icon-error'"
                            :style="{color:scope.row.reserve=='已预订'?'#10CF46':'#f56c6c',fontSize:'17px'}"
                          ></i>
                        </el-button>
                      </el-tooltip>
                    </template>
                  ...
                  <el-table-column label="报名" props="entry" align="center" min-width="12%">
                    <template slot-scope="scope">
                      <el-tooltip
                        class="item"
                        :disabled="!scope.row.entry_txt"
                        effect="dark"
                        :content="scope.row.entry_txt"
                        placement="top"
                      >
                        <el-button
                          type="text"
                          style="color:#606266"
                          @click="onShowTk('bmxx_txt',scope.row)"
                        >
                          <i
                            :class="scope.row.entry=='已报名'?'el-icon-circle-check':'el-icon-error'"
                            :style="{color:scope.row.entry=='已报名'?'#10CF46':'#f56c6c',fontSize:'17px'}"
                          ></i>
                        </el-button>
                      </el-tooltip>
                    </template>
                  ...
                  <el-table-column width="100" label="遇到的问题" align="center" prop="difficult_txt">
                    <template slot-scope="scope">
                      <el-tooltip
                        class="item"
                        :disabled="!scope.row.difficult_txt"
                        effect="dark"
                        :content="scope.row.difficult_txt"
                        placement="top"
                      >
                        <el-button
                          type="text"
                          style="color:#606266"
                          @click="onShowTk('difficult_txt',scope.row)"
                        >
                          <i
                            :class="scope.row.difficult?'el-icon-circle-check':'el-icon-error'"
                            :style="{color:scope.row.difficult?'#10CF46':'#f56c6c',fontSize:'17px'}"
                          ></i>
                        </el-button>
                      </el-tooltip>
                    </template>
                  ...
                  <el-table-column width="100" label="积分" align="center" prop="jf">
                    <template slot-scope="scope">
                      <el-button
                        type="text"
                        @click="jfFn().onShow(scope.row)"
                        :style="{color:scope.row.integral<0?'red':''}"
                      >{{scope.row.integral}}</el-button>
                    </template>
                  </el-table-column>
                  <el-table-column width="100" label="留言" align="center" prop="ly">
                    <template slot-scope="scope">
                      <el-button
                        type="text"
                        @click="lyFn().onShow(scope.row)"
                      >{{scope.row.count_words}}</el-button>
                    </template>
                  </el-table-column>
              ...
              <div :style="tableStyleR" @click="checkTable(2)">
                ...
                <el-table :data="noTableDataFirst.rows" v-loading="loading2" border>
                  <el-table-column label="序号" header-align="center" align="center" width="100">
                    <template slot-scope="scope">{{scope.$index+(pageR-1)*10+1}}</template>
                  </el-table-column>
                  <el-table-column prop="name" label="企业名称" min-width="40%" align="center">
                    <template slot-scope="scope">
                      <el-button @click="open_gs(scope.row)" type="text">{{scope.row.gsmc}}</el-button>
